...
abstract class Base {
	 ...
	 * @var array
	 */
	protected $_called_doc_hooks = array(); // phpcs:ignore PSR2.Classes.PropertyDeclaration.Underscore
	 ...
	 * @return mixed
	 */
	protected function _add_hook( $type, $name, $callback, $args = array() ) { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
	...
	public function add_doc_hooks( $object = null ) {
		...
		if ( isset( $this->_called_doc_hooks[ $class_name ] ) ) {
			...
			if ( ! $this->is_wpcom_vip_prod() ) {
				trigger_error( esc_html( $notice ), \E_USER_NOTICE ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_trigger_error
			}
			return;
		}
